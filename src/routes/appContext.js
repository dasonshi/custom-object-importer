  ...
  try {
    ...
    if (locationToCheck && !await installs.has(locationToCheck) && user?.companyId) {
      ...
      if (pendingAgency) {
        ...
        // No agency installation found - cannot recover automatically
        // GHL's Reconnect API was removed, so user must reinstall via OAuth
        console.log('❌ No installation found for location:', locationToCheck);
        console.log('🔄 Redirecting to OAuth install flow...');
